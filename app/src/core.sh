...
# GETRUNNINGAPPINFO: get the PID and canonical path of a running app
function getrunningappinfo {  # ( path varPID varCanonPath )
    ...
    # result variables
    local varPID="$1" ; shift
    local varCanonPath="$1" ; shift
    ...
    if [[ ! -d "$path" ]] ; then
	ok=
	errmsg="Unable to get canonical path for '$1'."
	errlog "$errmsg"
    fi
    
    # get ASN associated with this bundle path
    local asn=
    try 'asn=' /usr/bin/lsappinfo find "bundlepath=$path" \
	"Error attempting to find running app at '$path'."
	...
	try 'info=' /usr/bin/lsappinfo info -only pid "$asn" \
	    "Error getting PID for running app at '$path'"
    fi
	
    # if this ASN matches our bundle, grab the PID & path
    if [[ "$ok" ]] ; then
	re='^"pid" *= *([0-9]+)$'
	if [[ "$info" =~ $re ]] ; then
	    resultPID="${BASH_REMATCH[1]}"
	    resultPath="$path"
	    
	    debuglog "Found running app at '$resultPath' with PID $resultPID."
	fi
    fi
    
    # write to result variables (whether success or failure)
    eval "${varPID}=\"\$resultPID\""
    eval "${varCanonPath}=\"\$resultPath\""
    
    # return result
    if [[ "$resultPID" ]] ; then
	return 0
    elif [[ "$ok" ]] ; then
	debuglog "No running app found at '$path'."
	return 0
    else
