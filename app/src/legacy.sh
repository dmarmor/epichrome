...
function updateolddatadir {  # ( [locDataPath locProfilePath] )
    ...
    if [[ ( -d "$locDataPath" ) && ! -d "$locProfilePath" ]] ; then
        ...
        if [[ "$ok" ]] ; then
            ...
            local allExcept="!($appDataProfileDir|$epiDataLogDir|$appDataStdoutFile|$appDataStderrFile|$appDataErrmsgFile|$appDataPauseFifo|$appDataBackupDir|$appDataWelcomeDir)"
