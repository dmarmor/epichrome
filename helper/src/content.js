...
ssbContent.contextMenuEndHandler = function(evt) {
    ...
    ssbContent.sendInteraction(evt, ssbContent.getInteractionInfo(),
			       'contextmenu', true);
}
...
ssbContent.sendInteraction = function(evt, info, type, isLate) {
    ...
    ssb.debug(message.info ? message.info.type.toUpperCase() : 'CLEAR', 'at:',
...
ssbContent.linkHandler = function(evt, info) {
	...
	default:
	    ...
	    target =
		((sameDomain && topWindow.document.querySelector('iframe'+frameSelector+
								 ',frame'+frameSelector)) ?
		 false :
		 'external');
	}
	...
	ssb.debug('linkHandler', 'requesting redirect --',
		  ' url:',href,'['+target+']');
...
ssbContent.bootstrap = function() {
	...
	if (ssbContent.bootstrapCount < 3) { // 1.5 seconds after load attempt
	    ...
	    console.debug('[bootstrap]:', 'giving up -- shared.js not loaded after 1.5 seconds', (window.top == window) ? '(top frame)' : '');	    
    }
}
