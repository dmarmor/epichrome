...
ssbBG.startup = function() {
    ...
    ssb.startup('background', function(success, message) {
        if (success) {
            ...
            ssbBG.contextMenuCreate(
                'epichrome', 'separator1',
                undefined, ['link'], 'separator'
            );
            ssbBG.contextMenuCreate(
                'epichrome', 'urlThisAppNewWindow',
                'Open in New App Window (⌥⇧-click)', ['link']
            );
            ssbBG.contextMenuCreate(
                'epichrome', 'urlThisAppNewTab',
                'Open in New Tab (⌘⌥-click)', ['link']
            );
            ssbBG.contextMenuCreate(
                'epichrome', 'urlThisAppSameTab',
                'Open in Same Window (⌘⌥⇧-click)', ['link']
            );
            ssbBG.contextMenuCreate(
                'epichrome', 'separator2',
                undefined, ['link'], 'separator'
            );
            ssbBG.contextMenuCreate(
                                    ...
                                    chrome.tabs.sendMessage(tab.id, 'ping', function(response) {
                                        ...
                                        if (response != 'ping') {
                                            ...
                                            if (chrome.runtime.lastError) {
                                                ssb.debug(
                                                    'initTabs',
                                                    'ping result for tab', tab.id,
                                                    ':',chrome.runtime.lastError.message
                                                );
                                            } else {
                                                ssb.debug(
                                                    'initTabs',
                                                    'ping result for tab', tab.id, ':', response
                                                );
                                            }
                                            ...
                                            for(var i = 0 ; i < scripts.length; i++ ) {
                                                ...
                                                ssb.debug(
                                                    'initTabs',
                                                    'injecting', scripts[i], 'into tab', tab.id
                                                );
                                            ...
                                            ssb.debug(
                                                'initTabs',
                                                'NOT reloading tab', tab.id,'-- got ping'
                                            );
                                        }
                                    });
    ...
    chrome.windows.getAll( { populate: true }, function (windows) {
        ...
        for(var i = 0 ; i < windows.length; i++ ) {
            ...
            for(var j = 0 ; j < curWindow.tabs.length; j++ ) {
                ...
                if (
                    ! (ssb.regexpChromeScheme.test(curTab.url) ||
...
ssbBG.navTargetCreatedHandler = function(details) {
    ...
    // $$$ TESTING
    if (
        !ssb.regexpChromeScheme.test(details.url) &&
        ...
        if ((interactToNewTab >= 0) && (interactToNewTab < 200) && (newTabToWebNav < 200)) {
            ...
            if (details.url == 'about:blank') {
                ...
                ssb.debug(
                    'navTarget',
                    'CAPTURE (', interactToNewTab, '+', newTabToWebNav, ')', details
                );
            ...
            // $$$ KILL THIS
            ssb.debug(
                'navTarget', 'IGNORE -- bad interaction/newTab times (',
        ...
        if (ssb.regexpChromeScheme.test(details.url)) {
            ...
            ssb.debug(
                'navTarget', 'IGNORE -- newTab already has URL',
                ssbBG.events.newTab, details
            );
        } else if (details.sourceTabId != ssbBG.events.interaction.tabId) {
            ssb.debug(
                'navTarget', 'IGNORE -- navTarget sourceTabId not interaction tab (',
                details.sourceTabId, '/', ssbBG.events.interaction.tabId, ')', details
            );
        } else {  // if (details.tabId == ssbBG.events.newTab.tab.id)
            ssb.debug(
                'navTarget', 'IGNORE -- navTarget not in same tab as newTab (',
                details.sourceTabId, '/', ssbBG.events.interaction.tabId, ')', details
            );
        }
    }
            ...
            if (ssbBG.events.navTarget) {
                ...
                if ((navTargetToRequest >= 0) && (navTargetToRequest < 500)) {
                    ...
                    if (details.tabId == ssbBG.events.navTarget.details.tabId) {
                        ...
                        ssb.debug(
                            'beforeRequest',
                            'CAPTURE late navTarget', details, ssbBG.events.navTarget
                        );
                        
                        // determine if we should redirect this tab
                        ssbBG.redirectNewTab(
                        ...
                        ssb.debug(
                            'beforeRequest',
                            'IGNORE late navTarget -- different tab from target (',
                    ...
                    ssb.debug(
                        'beforeRequest', 'IGNORE late navTarget -- too late',
                ...
                if (
                    handled || killNavTarget || !ssbBG.events.interaction ||
                    ...
                    if (!handled && !killNavTarget) {
                        if (!ssbBG.events.interaction) {
                            ssb.debug(
                                'beforeRequest',
                                'KILL late navTarget -- no interaction found',
                                details
                            );
                        } else {  // if interaction time > navTarget time
                            ssb.debug(
                                'beforeRequest',
                                'KILL late navTarget -- interaction after navTarget',
                                details
                            );
                        }
                    }
                ...
                ssb.debug(
                    'beforeRequest',
                    'IGNORE late navTarget -- no target found',
                    details
                );
            }
            ...
            if (!handled) {
                if (
                    ...
                    if ((interactToRequest >= 0) && (interactToRequest < 200)) {
                        ...
                        // run rules here
                        if (ssb.shouldRedirect(
                            details.url,
                            'internal', ssbBG.events.interaction.classList
                        )) {
                        ...
                        // $$$$ KILL THIS
                        ssb.debug(
                            'beforeRequest', 'IGNORE -- bad interaction time',
                    ...
                    if (!ssbBG.events.interaction) {
                        ...
                        ssb.debug(
                            'beforeRequest', 'IGNORE -- different tab from interaction (',
